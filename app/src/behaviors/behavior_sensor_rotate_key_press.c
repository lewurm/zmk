...
static int behavior_sensor_rotate_key_press_init(const struct device *dev) {
    ...
    if (init_first_run) {
        ...
        for (int i = 0; i < ZMK_BHV_SENSOR_MAX_MODS; i++) {
            k_delayed_work_init(&active_mod_presses[i].work, behavior_sensor_rotate_key_press_work_handler);
...
#define KP_INST(n)                                                                                 \
    ...
    DEVICE_DT_INST_DEFINE(n, behavior_sensor_rotate_key_press_init, device_pm_control_nop,         \
                          NULL, &behavior_sensor_rotate_key_press_config_##n, APPLICATION,         \
